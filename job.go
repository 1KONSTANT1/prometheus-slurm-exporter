...
func ParseJobMetrics(input []byte) map[string]*JobsMetrics {
	...
	for _, line := range linesUniq {
		if strings.Contains(line, "|") {
			...
			jobs[jobid].partition = split[16][:len(split[16])-1]
...
func JobData() []byte {
	cmd := exec.Command("squeue", "-a", "-r", "-h", "-o \"%A|%V|%S|%e|%l|%L|%M|%T|%r|%u|%g|%Q|%N|%C|%m|%a|%P\"")
	out, err := cmd.Output()
	if err != nil {
		log.Fatal(err)
	}
...
func NewJobCollector() *JobCollector {
	labels := []string{"JOBID", "SUBMIT_TIME", "START_TIME", "END_TIME", "TIME_LIMIT", "STATUS", "USER", "GROUP", "PRIORITY", "RUN_TIME", "NODELIST", "CPUS", "MIN_MEM_REQUSTED", "ACCOUNT", "PARTITION"}
...
func (nc *JobCollector) Collect(ch chan<- prometheus.Metric) {
	jobs := JobGetMetrics()
	for job := range jobs {
		ch <- prometheus.MustNewConstMetric(nc.time, prometheus.GaugeValue, float64(0), job, jobs[job].sub_time, jobs[job].start_time, jobs[job].end_time, jobs[job].time_limit, jobs[job].status, jobs[job].user, jobs[job].group, jobs[job].priority, jobs[job].run_time, jobs[job].nodes, jobs[job].cpus, jobs[job].min_mem, jobs[job].account, jobs[job].partition)
	}
}
