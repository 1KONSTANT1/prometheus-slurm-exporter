...
func ParseJobMetrics(input []byte) (map[string]*JobsMetrics, map[string]*CompletedJobsMetrics) {
	...
	for _, line := range linesUniq {
		if strings.Contains(line, "|") {
			...
			jobs[jobid].tres_alloc = split[19]
			jobs[jobid].partition = strings.Fields(split[20])[0]
...
func JobData() []byte {
	cmd := exec.Command("/bin/bash", "-c", "squeue -a -r -h -O \"JOBID:|,SubmitTime:|,STARTTIME:|,ENDTIME:|,TIMELIMIT:|,TIMELEFT:|,TIMEUSED:|,STATE:|,REASON:|,USERNAME:|,GroupNAME:|,PRIORITYLONG:|,NODELIST:|,NumCPUs:|,MinMemory:|,ACCOUNT:|,ReasonList:|,MinTmpDisk:|,tres-per-node:|,tres-alloc:|,PARTITION\"")
	out, err := cmd.Output()
	if err != nil {
		log.Fatal(err)
	}
...
func NewJobCollector() *JobCollector {
	labels := []string{"JOBID", "SUBMIT_TIME", "START_TIME", "END_TIME", "TIME_LIMIT", "STATUS", "USER", "GROUP", "PRIORITY", "RUN_TIME", "NODELIST", "CPUS", "MIN_MEM_REQUSTED", "ACCOUNT", "PARTITION", "REASON", "MIN_TMP_DISK", "TRES_PER_NODE", "TRES_ALLOC"}
